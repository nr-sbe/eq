...
test('weakness grants fifty percent bonus to powers and infused sword, including guardians; shielding is preserved',()=>{
 for(let el=0;el<5;el++)for(const sword of [false,true]){const g=arena();g.element=el;const e=enemy(g,0,sword?3:20);e.weakness=el;g.cast(e,false,sword);assert.equal(e.max-e.hp,(sword?22:8)*1.5);assert.equal(g.events.find(e=>e.type==='damage').weak,true);}
