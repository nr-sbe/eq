...
class StaticServerTests(unittest.TestCase):
    def test_entry_and_audio(self):
        for url in ['/','/Fivefold-3D.html','/game.js','/fullscreen.js','/sword-fx.js','/manifest.json','/assets/app-icon.png','/assets/audio/old-ones.ogg','/assets/effects/lightning-impact.wav','/assets/effects/lightning-impact.wav']:
