...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory;else factory(root.SevenWorlds);})(typeof window==='object'?window:this,function(api){
 ...
 for(const level of api.levels){level.signature={title:titles[level.id],element:required[level.id]};level.encounterBeats=level.encounters.map((z,id)=>({id,z,kind:id===1?'signature':'combat'}));level.environmentInteractions=[{...level.signature,z:level.id===0?480:290}];level.exclusionZones=level.id===2?[{z:level.bossTrigger-12,end:level.length}]:[];level.combat=['japan-battle.mp3','west-battle.mp3','ai-fight.mp3','egypt-battle.mp3','legionnaire.mp3','gothic-battle.mp3','heavens-battle.mp3'][level.id];level.soundtrack={exploration:level.music,combat:level.combat};}
