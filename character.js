...
(()=>{
 ...
 function createWarden(template,clips,scale,index,theme='japan',role='melee'){
  ...
  m.traverse(o=>{if(o.isBone)bones[o.name.replace(/[^a-zA-Z0-9]/g,'')]=o;if(o.isSkinnedMesh){o.castShadow=o.receiveShadow=true;o.frustumCulled=false;
   ...
   o.material=o.material.clone();o.material.color.set({japan:'#40363b',west:'#625343',cyber:'#263944',egypt:'#948767',ice:'#788994',gothic:'#3b3539',celestial:'#777080'}[theme]);o.material.roughness=.8;o.material.metalness=.3;
  ...
  const chest=plate([[-.38,.16],[0,.3],[.38,.16],[.28,-.62],[0,-.85],[-.28,-.62]],.15,iron,mantle);chest.position.z=.34;
  ...
  if(role==='shield'){const shield=mesh(new T.BoxGeometry(.8,1.4,.16),iron,mantle);shield.position.set(-.8,-.25,.6);const strip=mesh(new T.BoxGeometry(.06,1,.18),glow,shield);}
 ...
 function createActionHero(){
  ...
  },makeEnemy(index,theme,role){if(theme==='cyber'&&role==='ranged')return createDrone(index);return createWarden(template,clips,scale,index,theme,role);}};
 }
