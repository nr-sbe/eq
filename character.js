...
(()=>{
 ...
 function createWarden(template,clips,scale,index,theme='japan',role='melee',weaknessColor=null){
  ...
  if(role==='boss'&&theme==='cyber'){const core=mesh(new T.IcosahedronGeometry(.22,1),glow,mantle);core.position.set(0,-.15,.58);const halo=mesh(new T.TorusGeometry(.65,.055,6,40),glow,head);halo.position.set(0,.25,-.24);for(const side of [-1,1]){const strip=mesh(new T.BoxGeometry(.055,.65,.08),glow,mantle);strip.position.set(side*.48,-.1,.39);}}
  ...
  return {root:r,update(t,moving,state={}){const stunned=(state.status?.stun||0)>(state.simTime||0),windup=Math.max(0,(state.attackUntil||0)-(state.simTime||0)),recoil=state.hit||0;mx.update(stunned?0:Math.min(.05,Math.max(0,t-tick))*(recoil? .25:1));tick=t;if(stunned)moving=false;idle.setEffectiveWeight(moving?.1:1);run.setEffectiveWeight(moving?.9:0);r.updateMatrixWorld(true);place(head,'Head',.15);place(mantle,'Spine2',-.12);place(pole,'RightHand',.35);pole.rotation.z=stunned?.02:.16;const strike=windup>0?1-Math.min(1,windup/(state.role==='boss'?1.2:.9)):0;mantle.rotation.y=windup>0?-Math.sin(strike*Math.PI)*.45:0;mantle.rotation.x=recoil*1.3;pole.rotation.x=role==='ranged'?-1.1:windup>0?-.8-strike*1.1:Math.sin(t*2)*.035;pole.rotation.z+=windup>0?Math.sin(strike*Math.PI)*1.2:0;head.rotation.x=stunned?-.15:recoil*.7;if((state.followThrough||0)>(state.simTime||0)){const f=1-((state.followThrough-state.simTime)/.3);pole.rotation.x=role==='ranged'?1.3:1.2-f*.5;pole.rotation.z=(role==='ranged'?.1:1-f*2);mantle.rotation.y=.55*(1-f);}
 ...
 function createActionHero(){
  const root=new T.Group(),sword=weapon();root.add(sword.root);sword.root.visible=false;let gripSocket=null;let template=null,clips=null,model=null,mixer=null,current=null,actions={},last=0,heldPose=null,bones={},scale=1;const aimY=new T.Vector3(0,1,0),v=new T.Vector3(),q=new T.Quaternion(),parentQ=new T.Quaternion();
  const ready=new Promise((resolve,reject)=>{try{const binary=atob(window.FivefoldHeroData),bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);new window.GLTFLoader().parse(bytes.buffer,'',gltf=>{
   ...
   model.traverse(o=>{if(o.isMesh){o.castShadow=o.receiveShadow=true;o.frustumCulled=false;o.material=o.material.clone();o.material.roughness=Math.max(.4,o.material.roughness||.5);o.material.color.set('#46545e');if(o.name.includes('visor')){o.material.transparent=false;o.material.opacity=1;o.material.color.set('#263c49');o.material.metalness=.8;o.material.roughness=.22;}}if(o.isBone)bones[o.name.replace(/[^a-zA-Z0-9]/g,'')]=o;});
   mixer=new T.AnimationMixer(model);for(const clip of clips)actions[clip.name]=mixer.clipAction(clip);current=actions.Idle;current.play();mixer.update(.01);root.updateMatrixWorld(true);const hand=bone('RightHand');if(hand){gripSocket=new T.Group();gripSocket.name='SwordPalmSocket';gripSocket.position.set(-2,12,-1);hand.add(gripSocket);gripSocket.add(sword.root);sword.root.name='HeldSword';sword.root.quaternion.setFromUnitVectors(aimY,new T.Vector3(0,0,1));}sword.root.visible=true;resolve();
  ...
  return {root,ready,swordSegment(){sword.root.updateWorldMatrix(true,false);return {base:sword.root.localToWorld(new T.Vector3(0,.2,0)),tip:sword.root.localToWorld(new T.Vector3(0,1.94,0))};},hands(){return ['LeftHand','RightHand'].map(name=>{const b=bone(name);return b?b.getWorldPosition(new T.Vector3()):null;}).filter(Boolean);},update(t,p,casting,element){if(!mixer)return;const dt=Math.min(.05,Math.max(0,t-last));last=t;const next=p.moving>.1?actions.Run:actions.Idle;if(current!==next){next.reset().play();current.crossFadeTo(next,.18,false);current=next;}mixer.update(p.hold>0?0:dt*(p.moving?1.65:1));root.updateMatrixWorld(true);
   ...
   }if(casting){aim('LeftArm',new T.Vector3(-.35,1.8,2.2));aim('LeftForeArm',new T.Vector3(-.3,1.75,3));if(!striking){aim('RightArm',new T.Vector3(.6,1.6,1.2));aim('RightForeArm',new T.Vector3(.6,1.55,2));}}
   const hand=bone('RightHand');if(hand&&gripSocket){
    // Turn the wrist with the grip, then let the skeleton carry the weapon.
    hand.getWorldPosition(v);root.worldToLocal(v);
    const direction=pose?new T.Vector3(...pose.tip).sub(v).normalize():new T.Vector3(0,1,0).applyEuler(new T.Euler(casting?1.35:-.25,0,casting?.35:.48));
    const desired=direction.clone().transformDirection(root.matrixWorld),worldQ=hand.getWorldQuaternion(new T.Quaternion()),across=new T.Vector3(0,0,1).applyQuaternion(worldQ);
    const correction=new T.Quaternion().setFromUnitVectors(across,desired);worldQ.premultiply(correction);hand.parent.getWorldQuaternion(parentQ);hand.quaternion.copy(parentQ.invert().multiply(worldQ));
    for(const finger of ['Index','Middle','Ring','Pinky'])for(let joint=1;joint<=3;joint++){const b=bone('RightHand'+finger+joint);if(b)b.quaternion.setFromAxisAngle(new T.Vector3(0,0,1),joint===1?.85:1.15);}
    const thumb=bone('RightHandThumb2');if(thumb)thumb.quaternion.setFromAxisAngle(new T.Vector3(0,1,0),-.65);
    root.updateMatrixWorld(true);
    // Cancel the model's import scale, while preserving the hero's gameplay scale.
    const heroScale=root.getWorldScale(new T.Vector3()),socketScale=gripSocket.getWorldScale(new T.Vector3());sword.root.scale.copy(heroScale.divide(socketScale));
    const gripOffset=new T.Vector3(0,-.05,0).multiply(sword.root.scale).applyQuaternion(sword.root.quaternion);sword.root.position.copy(gripOffset).negate();
    sword.root.updateMatrixWorld(true);
   }const tint=p.enchanted===false?'#82b8ce':Fivefold3D.elements[element].color;sword.energy.color.set(tint).multiplyScalar(striking?2.6:1.6);sword.steel.emissive.set(tint).multiplyScalar(p.enchanted===false?0:striking?.42:.16);
