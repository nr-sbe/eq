...
Regular casts hit groups from Tier 1: widening air/fire fronts, a broad water lane, an earth corridor and clustered lightning. Tempests cover a wider area. Cover and closed gates block attacks. Enemy armor, cloaks, eyes and weapon accents are color-coded by weakness using the power selector palette: pale green for Air, orange for Fire, purple for Lightning, cyan for Water and ochre for Earth. This includes guardians and drones. Every enemy also has its elemental weakness shown below its target marker: **+50% damage**, including infused sword attacks. Shield guards are weak to Earth, ranged units to Air, supports to Lightning, melee variants to Fire or Water, and each guardian has a setting-specific weakness. Grid Executor relay protection remains intact.
