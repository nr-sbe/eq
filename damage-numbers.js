...
(function(root){'use strict';
 class DamageNumbers{
  ...
  event(e,time){if(e.type!=='damage'||!(e.amount>0)||!Number.isFinite(e.amount))return;
   ...
   if(label){label.amount+=e.amount;label.critical=label.critical||e.critical;}
   else{const sameEnemy=this.labels.filter(l=>l.active&&l.id===e.id);label=sameEnemy.length>=2?sameEnemy.reduce((a,b)=>a.birth<b.birth?a:b):this.labels.find(l=>!l.active)||this.labels.reduce((a,b)=>a.birth<b.birth?a:b);Object.assign(label,{active:true,id:e.id,element:e.element,amount:e.amount,critical:!!e.critical,birth:time,drift:(this.sequence++%2?1:-1)*18});}
   Object.assign(label,{x:e.x,y:e.y??3.2,z:e.z});label.el.textContent=String(Math.max(1,Math.round(label.amount)));label.el.style.color=['#d9f1e7','#ffc491','#dfc2ff','#b1edff','#ecd4ab'][e.element]||'#fff0d8';label.el.className='damage-number'+(label.amount>=60?' heavy':'')+(label.critical?' critical':'');
  }
