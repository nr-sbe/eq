...
(()=>{
 class TouchControls{
  constructor({canvas,onLook,onCycle,onDash}){
   ...
   for(const button of this.root.querySelectorAll('[data-touch]')){
    button.addEventListener('pointerdown',e=>{if(!this.enabled)return;e.preventDefault();button.setPointerCapture(e.pointerId);this.actions.set(e.pointerId,button.dataset.touch);button.classList.add('pressed');if(button.dataset.touch==='target')onCycle();if(button.dataset.touch==='dash')onDash();this.sync();});
